...
def WTfilt_1d(sig):
    ...
    """
    coeffs = pywt.wavedec(sig, 'db6', level=9) # multilevel decomposition，db6表示小波，9表示解码水平
    coeffs[-1] = np.zeros(len(coeffs[-1]))
    coeffs[-2] = np.zeros(len(coeffs[-2]))
    coeffs[0] = np.zeros(len(coeffs[0]))
    sig_filt = pywt.waverec(coeffs, 'db6') #multilevel reconstrucing
    return sig_filt
...
def SegSig_1d(sig, seg_length=1500, overlap_length=0,
    ...
    # 按指定参数对单导联ECG进行切片
    :param sig: 1-D numpy Array，单导联ECG
    :param seg_length:  int，切片的采样点长度，即切片长度
    ...
    start = stt
    while start+seg_length <= length: #当余下的信号还能够进行一次切片时
        tmp = sig[start:start+seg_length].reshape([1, seg_length])
        SEGs = np.concatenate((SEGs, tmp))#拼接到结果上
        start += seg_length #开始位置右移，为下一次切片做准备
        start -= overlap_length #需要减去覆盖部分的长度
    if full_seg: # 对信号末尾不足seg_length的片段进行延拓并切片
        if start < length:
            pad_length = seg_length-(length-start) # 需要补充的长度
            tmp = np.concatenate((sig[start:length].reshape([1, length-start]),
                                sig[:pad_length].reshape([1, pad_length])), axis=1)
            SEGs = np.concatenate((SEGs, tmp)) #  数组拼接
    SEGs = SEGs[1:]
    return SEGs
...
def Stack_Segs_generate(sig, seg_num=24, seg_length=1500, full_seg=True, stt=0):
    ...
    """
    sig = WTfilt_1d(sig) #滤波
    if len(sig) < seg_length+seg_num:#如果信号长度不够
        sig = Pad_1d(sig, target_length=(seg_length+seg_num-1))#信号补零
        
    overlap_length = int(seg_length-(len(sig) - seg_length)/(seg_num-1))
    
    if (len(sig) - seg_length) % (seg_num-1) == 0:#正好能够切片，末尾不足的情况不存在
        full_seg = False
    ...
    del sig #删除变量
    SEGs = SEGs.transpose() #矩阵转置，得到堆叠的切片
...
def Fetch_Pats_Lbs_sLead(Pat_files, Path, target_lead=1, seg_num=24,
    ...
    for i in range(len(Pat_files)):
        sig = np.load(Path+Pat_files[i])[target_lead, :] #加载数据
        SEGt = Stack_Segs_generate(sig, seg_num=seg_num,
                 seg_length=seg_length, full_seg=full_seg, stt=stt) # 滤波切片并堆叠为矩阵
...
def oversample_balance(records, labels, rand_seed):
    ...
    """
    class_num = len(np.unique(labels)) #类的个数
    num_records = len(records)#记录名集合的长度
    num_categories = []
